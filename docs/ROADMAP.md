...
- **Phase 9 — real scientific validation:** protocol gate prepared but not scientifically complete. The repository has frozen-scope contracts, independent-family verification, reporting, and an explicit `AWAITING_REAL_EVIDENCE` workspace status. No real Evo2 checkpoint output or experimentally evaluated regulatory-variant cohort is available, so no scientific result is claimed.
...
- **Post-Phase 10 readiness:** the frozen `StudyProtocol`/`ProtocolGate` and strict `RealEvo2Scorer` adapter are implemented and tested. They make the remaining scientific work executable only after a licensed, experimentally evaluated cohort and compatible checkpoint runner are supplied. SAE inspection, measured Qwen-in-colony execution, cross-model comparisons, collaboration, and scale-out remain gated extensions.
