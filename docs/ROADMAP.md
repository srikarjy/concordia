...
- **Phase 5 — seed scientist runtime:** software slice implemented; model qualification remains open. Four strict turn contracts, a local Ollama adapter, versioned genomic prompts, bounded tool execution, complete response preservation, reference checks, and deterministic qualification metrics are implemented. The command preserves failures and reuses saved completed executions. Under the declared two-repetition JSON-mode protocol, `phi3:latest` timed out twice and `gemma3:270m` produced truncated JSON twice; neither candidate was selected. Initial union-grammar transport failures are also retained. Completion requires a local model to pass the declared task and qualification thresholds. See [the qualification guide](seed-scientist.md).
- **Phase 6 and later:** not started. Colony evolution follows successful seed qualification.
