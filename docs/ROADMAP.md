...
- **Phase 8 — interactive workspace:** complete for the saved fixture demonstration. The React/TypeScript/Vite workspace, read-only FastAPI graph/artifact/event/report APIs, sequence and counterfactual tracks, claim-path highlighting, colony member inspection, event replay controls, keyboard-accessible graph fallback, reduced-motion styling, and production build are implemented. The browser renders only persisted fixture artifacts.
- **Phase 9 — real scientific validation:** inputs and protocol are frozen for a two-variant HBB promoter pilot, but outcome collection is not complete. The repository pins ClinVar identities, validated GRCh38 reference windows, data-use terms, Evo2 checkpoint/target, and evidence-family rules before model outcomes. A checksum-validated K562 DNase source and both in-window peaks are preserved, including no direct overlap at either variant base. No real Evo2, SAE, or comparator result is available, so no support status or scientific finding is claimed.
- **Phase 10 — hardening and portfolio release:** complete for the local software release. CI, reproducibility checks, deployment instructions, a CPU Docker image, and the public free Hugging Face Space are available. Real scientific validation remains a separate gated study and is not represented by the fixture deployment.
- **Post-Phase 10 readiness:** a strict real-Evo2 boundary, a non-scientific NVIDIA hosted-generation smoke test, SAE and cross-model validity contracts, measured local-Qwen colony execution, bounded CELLxGENE ingestion, a pinned official State source verifier, and an Arc State-compatible virtual-cell sandbox contract are implemented. The undocumented hosted-forward assumption was removed; the frozen Evo2 study still requires a verified local NIM `/forward` execution. A real 1,789-cell perturbation H5AD can enter content-addressed storage, but its State output remains a recorded fixture. Real Evo2/SAE/comparator/State output collection, checkpoint-feature qualification, collaboration, and measured scale-out remain gated extensions.
